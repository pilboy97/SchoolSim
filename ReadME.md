...
SchoolSim is a simulation where NPC students live autonomously inside a school environment.

Each student continuously evaluates their internal needs and the surrounding world to decide what to do next.  
As a result, emergent behaviors appear naturally such as:

- Students gathering to chat
- A tired student leaving class to rest
- Social students actively searching for friends

The goal of the project is not direct player control but observing **emergent behavior** created by the Utility AI system.
...
2. **Apply Need Modifiers (欲求ごとの重み付け):** Modifies the base score using mathematical curves based on the need type:
   ...
   The final score is calculated as:
    Score = (Need Utility × Personality Modifier × Action Effect) / Distance Penalty
...
4. **Weighted Random Selection (重み付きランダム選択):** Filters the top 6 actions and picks one using a weighted random selection to ensure natural, non-mechanical behavior.
    Instead of always choosing the highest score action, the system selects from the top 6 actions using weighted randomness.
    This approach avoids deterministic AI behavior and creates emergent and varied NPC interactions.
...
Below is the core scoring function used in the Utility AI system.
It calculates the final action score by combining need urgency, personality modifiers, and distance penalties.
...
To tune the Utility AI parameters objectively, I designed several Python visualization tools.
(Honestly, I DESIGNED these tool, created by AI)
These tools helped me analyze:

- Stability of NPC needs over time
- Balance between different need types
- Impact of relationship mechanics
